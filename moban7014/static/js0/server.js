...
app.use(cors()); // 允许跨域请求
app.use(express.static('D:\\newapp\\民族数据\\moban7014\\moban7014')); // 确保路径正确
...
app.post('/api/posts', (req, res) => {
    const { title, content, performer } = req.body;
    pool.query('INSERT INTO blog_posts (title, content, performer) VALUES (?, ?, ?)', [title, content, performer], (error, results) => {
        if (error) {
            res.status(500).send(error);
        } else {
            res.json({ id: results.insertId, title, content, performer });
        }
    });
});
// API路由 - 创建新演员帖子
...
app.delete('/api/posts/:id', (req, res) => {
    ...
    pool.query('DELETE FROM blog_posts WHERE id = ?', [postId], (error, results) => {
        if (error) {
            res.status(500).send(error);
        } else {
...
app.put('/api/posts/:id', (req, res) => {
    const postId = req.params.id;
    const { title, content, performer } = req.body;
    pool.query('UPDATE blog_posts SET title = ?, content = ?, performer = ? WHERE id = ?', [title, content, performer, postId], (error, results) => {
        if (error) {
            res.status(500).send(error);
        } else {
